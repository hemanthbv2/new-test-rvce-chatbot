...
const KB = {
    ...
    faculty: {
        ...
        me: [
            { n: "Dr. Shanmukha Nagaraj", u: "https://rvce.edu.in/department/me/faculty-bio/", d: "Professor, HOD & Dean (Student Affairs)", e: "23 years" },
...
allBranches.forEach(branch => {
    ...
    const kws = [
        `hod ${code}`, `${code} hod`, `head of ${code}`, `who is the hod of ${code}`,
        `hod ${name}`, `${name} hod`, `head of ${name}`
    ];
    
    // Branch-specific aliases
    if (code === 'cs') kws.push('hod cse', 'cse hod', 'computer science hod');
    if (code === 'ec') kws.push('hod ece', 'ece hod', 'electronics hod');
    if (code === 'ee') kws.push('hod eee', 'eee hod', 'electrical hod');
    if (code === 'me') kws.push('mechanical hod', 'mech hod');
    if (code === 'cv') kws.push('civil hod');

    branchHodIntents.push({ k: kws, id: `hod_${code}`, p: 1.5 });
});
...
allBranches.forEach(branch => {
    ...
    if (code === 'ec') kws.push('ece placement', 'ece placements');
    if (code === 'ee') kws.push('eee placement', 'eee placements');
    if (code === 'me') kws.push('mechanical placement', 'mech placement');
    if (code === 'cv') kws.push('civil placement');
                ...
                if (resp) {
                    ...
                    if (isLast && result.overflow && result.overflow.length > 0) {
                        ...
                        const overflowButtons = result.overflow.slice(0, 5).map(overflowId => {
                            ...
                            return { l: label, a: overflowId, i: '👉' };
                        });
                        
                        // Append a Back button at the end
                        overflowButtons.push({ l: 'Back', a: '_back', i: '⬅️' });
