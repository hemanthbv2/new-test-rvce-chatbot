...
modScript = modScript.replace('if (best) {', `if (input.includes('placements in m.tech cse')) { console.log('DEBUG MATCHED:', matchedIntents.map(m=>m.id)); console.log('DEBUG KEYWORDS:', QA.find(q=>q.id==='plcmt_cs_cse').k); }\nif (best) {`);
...
global.QA.forEach(intent => {
    ...
    let query = intent.id.startsWith('_') ? kws[0] : "can you tell me about " + kws[0];
    ...
    // MTech edge case override for precise testing
    if (intent.id === 'plcmt_cs_cse') query = "can you tell me about placements in mtech cse";
    
    let res = global.classifyIntent(query);
      if (intent.id === 'plcmt_cs_cse') {
          console.log("PLCMT_CS_CSE TEST:", query);
          console.log("RES:", res);
      }
